...
migrateup1:
	migrate -path db/migration -database "postgresql://root:${POSTGRES_PASSWORD}@localhost:5432/simple_bank?sslmode=disable" -verbose up 1
...
migratedown1:
	migrate -path db/migration -database "postgresql://root:${POSTGRES_PASSWORD}@localhost:5432/simple_bank?sslmode=disable" -verbose down 1
...
.PHONY: createdb, postgres, dropdb, migrateup, migratedown, test, server, mock, migrateup1, migratedown1
